class Carro:
    ...
    def __init__(self, modelo : str, marca : str, cor : str,
                       odometro : float, motor : bool, __tanque : float, consumo_medio: float):
        self.modelo = modelo
        self.marca = marca
        self.cor = cor
    ...
    def ligar(self):
        if not self.motor_on and self.__tanque > 0:
            ...
            raise Exception("Erro: Motor já ligado! ou ta vazio.")
    ...
    def __str__(self):
        info = (f'Carro {self.modelo}, marca {self.marca}, '
                f'cor {self.cor}\n{self.__odometro} Km, '
                f'motor {self.motor_on} consumo {self.consumo_medio} km/l __tanque {self.__tanque} L')
        return info
